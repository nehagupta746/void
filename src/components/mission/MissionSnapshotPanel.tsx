...
function sourceStatus(loading: boolean): string { return loading ? "sync" : "live"; }
...
export function MissionSnapshotPanel() {
  ...
  </div><div className="mt-4 grid gap-2 border-t border-[var(--color-line)] pt-3 sm:grid-cols-2 lg:grid-cols-4">{(Object.keys(sourceLabels) as Array<keyof typeof sourceLabels>).map((source) => <div key={source} className="flex items-center justify-between gap-2 text-[0.625rem] uppercase tracking-[0.08em]"><span className="text-[var(--color-muted)]">{sourceLabels[source]}</span><span className="text-[var(--color-signal)]">{sourceStatus(loading)}</span></div>)}</div></HUDPanel>;
}